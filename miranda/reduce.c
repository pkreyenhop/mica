...
{ 
  extern word *cstack;
  /* GC root fence: this marks the current top-level expression as live
   * while output/reduction walks and mutates it. */
  cstack = &e; /* don't follow C stack below this in gc */
  e= reduce(e);
  while(tag[e]==CONS)
  { hd(e)= reduce(hd(e));
    /* Miranda encodes output effects as constructors (Stdout, Tofile, ...);
     * execute each message in order to realize side effects. */
