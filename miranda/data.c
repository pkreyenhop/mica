...
char *tag;
/* Heap model:
 * - tag[i] stores object kind
 * - hd(i)/tl(i) store payload words for cell i
 * - listp is the allocator scan cursor
 */
word files;
word current_file;
...
{ while(poschar(tag[++listp]))
    ;
       /* Find next reusable slot: positive tags are occupied/live. */
  if(listp==TOP)
    { if(SPACE!=SPACELIMIT)
        {
...
{ char *p1;
  ...
  /* setjmp() snapshots register state to memory so conservative stack/root
   * scanning won't miss live pointers currently resident in registers. */
...
void bindparams(word formal,word actual) /* process bindings of free ids */
